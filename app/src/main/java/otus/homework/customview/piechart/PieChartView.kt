...
class PieChartView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private val minTextSize = resources.getDimensionPixelSize(R.dimen.pie_chart_center_text_min_size)
    ...
    private var sections = listOf<PieChartSection>()
    ...
    private fun readAttributes(attrs: AttributeSet) {
        ...
        try {
            textPaint.textSize =
                typedArray.getDimensionPixelSize(R.styleable.PieChartView_centerTextSize, minTextSize).toFloat()
            textPaint.color = typedArray.getColor(R.styleable.PieChartView_centerTextColor, defaultColor)
            centerText = typedArray.getString(R.styleable.PieChartView_android_text) ?: ""
        } finally {
            typedArray.recycle()
        }
    }
    ...
    private fun calculatePieChartSections(
        ...
        val sections = mutableListOf<PieChartSection>()
        var currentAngle = 0F
    ...
    private fun getTouchPieChartAngle(touchX: Float, touchY: Float): Float? {
        ...
        val centerX = size / 2
        val centerY = size / 2
        Log.e("AAA", "$touchX, $touchY")

        val distanceBetweenCenterAndTouchPoint = sqrt((centerX - touchX).pow(2) + (centerY - touchY).pow(2))
    ...
    private fun animateOnSectionTouch(section: PieChartSection) {
        isAnimating = true
        AnimatorSet().apply {
            playTogether(
    ...
    private fun animateCenterText(section: PieChartSection): ValueAnimator {
    ...
    private class PieChartSavedState : BaseSavedState {
        ...
        companion object CREATOR : Creator<PieChartSavedState> {
