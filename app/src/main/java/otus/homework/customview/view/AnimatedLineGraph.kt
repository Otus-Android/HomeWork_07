...
class AnimatedLineGraph @JvmOverloads constructor(
  context: Context,
  attrs: AttributeSet? = null,
  defStyleAttrs: Int = R.attr.LineChartStyle,
  defStyleRes: Int = R.style.AnimatedLineGraph
  ...
  init {
    val a = context.obtainStyledAttributes(attrs, R.styleable.AnimatedLineGraph, defStyleAttrs, defStyleRes)
    configureAttr(a)
    a.recycle()
    configurePaints()
  ...
  private fun configureAttr(a: TypedArray) {
    a.run {
      lineColor = getColor(
        R.styleable. AnimatedLineGraph_linechart_lineColor,
        ContextCompat.getColor(context, R.color.purple_200)
      )

      fillColor = getColor(
        R.styleable.AnimatedLineGraph_linechart_fillColor,
        ContextCompat.getColor(context, R.color.purple_200)
      )

      lineWidth = getDimension(R.styleable.AnimatedLineGraph_linechart_lineWidth, 0f)
      cornerRadius = getDimension(R.styleable.AnimatedLineGraph_linechart_cornerRadius, 0f)
      isGradientFill = a.getBoolean(R.styleable.AnimatedLineGraph_linechart_gradientFill, false)
      animationDelay = getInt(R.styleable.AnimatedLineGraph_linechart_aimationDelay, 0)
      animateChanges = getBoolean(R.styleable.AnimatedLineGraph_linechart_animateChanges, false)
      fillType = getInt(R.styleable.AnimatedLineGraph_linechart_fillType, FillType.NONE)
    }
  }
  ...
  private fun populatePath() {
    ...
    for (i in 0 until adapter!!.count) {
      if (i < adapter!!.count) {
        ...
        linePath.lineTo(x + startX, abs(startY - y))
      }
    }
...
//    )
    contentRect.set(startX, 0f, width.toFloat() - 20f, height.toFloat() - 20f)
  }
