...
class LinearGraphView(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    private val textPaint = Paint().apply {
        color = Color.BLACK
        textSize =
            typedArray.getDimensionPixelSize(R.styleable.GraphView_text, 8.dPToPx()).toFloat()
    }
    ...
    override fun onSaveInstanceState(): Parcelable {
        ...
        return LinearGraphSavedState(pointList, state)
    }

    override fun onRestoreInstanceState(state: Parcelable?) {
        val ss = state as LinearGraphSavedState
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        when {
            widthMode == MeasureSpec.UNSPECIFIED && heightMode == MeasureSpec.UNSPECIFIED -> setMeasuredDimension(
                min,
                min
            )
            widthMode == MeasureSpec.UNSPECIFIED && heightMode == MeasureSpec.AT_MOST -> setMeasuredDimension(
                min,
                min
            )
            widthMode == MeasureSpec.UNSPECIFIED && heightMode == MeasureSpec.EXACTLY -> setMeasuredDimension(
                heightSize,
                heightSize
            )
            widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.UNSPECIFIED -> setMeasuredDimension(
                min,
                min
            )
            widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.AT_MOST -> setMeasuredDimension(
                min,
                min
            )
            widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.EXACTLY -> setMeasuredDimension(
                widthSize,
                heightSize
            )
            widthMode == MeasureSpec.EXACTLY && heightMode == MeasureSpec.UNSPECIFIED -> setMeasuredDimension(
                min,
                min
            )
            widthMode == MeasureSpec.EXACTLY && heightMode == MeasureSpec.AT_MOST -> setMeasuredDimension(
                widthSize,
                heightSize
            )
            widthMode == MeasureSpec.EXACTLY && heightMode == MeasureSpec.EXACTLY -> setMeasuredDimension(
                widthSize,
                heightSize
            )
        }
        ...
        canvas?.apply {
            with(rectF) {
                ...
                drawText(
                    ...
                    bottom + xAxisTextBounds.height() + 2f.dPToPx() - xAxisTextBounds.height() - 35,
                    textPaint
                )
                drawLine(
                    left + yAxisTextBounds.height(),
                ...
                drawText(
                    ...
                    top - 4f.dPToPx() + yAxisTextBounds.height() + 35,
                    textPaint
                )
                restore()
            }
